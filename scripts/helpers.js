...
const compile = async (contract, format) => {
  ...
  contracts.forEach(contract => {
    ...
    try {
      michelson = execSync(
        `${ligo} compile contract $PWD/${env.contractsDir}/${contract}.ligo -s pascaligo --michelson-format ${format}`,
        { maxBuffer: 1024 * 4000 },
      ).toString();
    } catch (e) {
      console.log(e);
...
const compileTest = async (contract, format) => {
  ...
  contracts.forEach(contract => {
    ...
    try {
      michelson = execSync(
        `${ligo} compile contract $PWD/${env.contractsTestDir}/${contract}.ligo -s pascaligo --michelson-format ${format}`,
        { maxBuffer: 1024 * 2000 },
      ).toString();
    } catch (e) {
      console.log(e);

...
# balance example
# ct.balance_of([{"owner": "KT1LzyPS8rN375tC31WPAVHaQ4HyBvTSLwBu", "token_id": 0}], None)
...
# TODO parse everything including Tez
...
def add_delimiters(d):
    res = {}
    for key,val in d.items():        
        if isinstance(val, dict):
            res[key] = add_delimiters(val)
        if isinstance(val, list):
            new_list = []
            for i in val:
                if isinstance(i, dict):
                    new_val = add_delimiters(i)

  ...
  then store
    ...
    const user_permits : user_permits =
      ...
      | None -> (failwith(err_message) : user_permits)
          ...
          None -> (failwith(err_message) : storage_type)
        | Some(permit_info) ->
            if has_expired(
                store.default_expiry,
                user_permits.expiry,
                permit_info)
            then (failwith("EXPIRED_PERMIT") : storage_type)
...
function add_permit(
  ...
  block {
    ...
    then store with record[
      ...
      const failwith_ : (string * bytes -> storage_type) =
        [%Michelson ({| { FAILWITH } |} : string * bytes -> storage_type)];
...
function set_expiry(
  ...
  block {
    ...
    const new_expiry : seconds = param.expiry;
    const specific_permit_or_default : option(blake2b_hash) = param.permit_hash;
